...
const genPoint = (x, y) => {
    ctx.beginPath();
    ctx.arc(x,y,2, 0, 5 * Math.PI);
    ctx.stroke();
};

const genFnHeader = (point, rname) => {
    let [tox, radius] = point;
    toy = -radius;

    ctx.beginPath();
    let ang = Math.atan((VENNHEIGHT - 25) / 66.666);
    ctx.arc(tox, 0, radius,-ang,Math.PI + ang, true);
    ctx.stroke();

    ctx.fillText(rname, tox -5, toy - 10);
...
function gen (domain, codomain, rel, count) {
    fnBasis(count);
...
function fnBasis (count = 2) {
    let ctx = clearCanvas();

    let arr = Array(count).fill(0).map((x, i) => (400 / (count + 1)) * (i + 1) -200);
    arr.map((x, i) => {
        ctx.beginPath();
        ctx.ellipse(x, 0, 30, VENNHEIGHT - 25, 0, 0, 2 * Math.PI);
        ctx.stroke();
    });
...
function canVM () {
    ...
    self.chapters = [
        { id: 1, name: 'Function', ctxClear: () => fnBasis(2), gen: () => self.fnGen(2) },
        { id: 3, name: 'Composite Function', ctxClear: () => fnBasis(3), gen: () => self.fnGen(3) },
    ...
    self.gen = ko.computed(() => {
        return self.chapter()
            ? self.chapters.find(x => x.id == self.chapter())?.gen
            : (() => {});
    });
